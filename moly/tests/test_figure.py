...
def test_add_molecule_from_file():
    fig = moly.Figure()
    mol = moly.Molecule.from_file("test_files/water.xyz")
...
def test_add_cube():
    fig = moly.Figure()
    fig.add_cube("test_files/Da.cube", iso=0.03, colorscale="rdbu", opacity=0.2)

def test_add_cube_slider():
    fig = moly.Figure()
    fig.add_cube("test_files/Da.cube", iso=[0.01, 0.1], colorscale="rdbu", opacity=0.2)
